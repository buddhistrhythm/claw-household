...
/**
 * load.js — 抓取 + 解析单个 feed 为原始条目，并打上 feed 身份
 * （source / source_name / category）。
 * Fetch + parse a single feed into raw items, stamped with the feed's
 * identity (source / source_name / category).
 */
...
async function readFeedBody(feed, rsshub) {
  if (feed.file) {
    // {file} 相对 lifeos 根目录解析。{file} is relative to the lifeos root.
...
module.exports = { loadFeed, readFeedBody };
